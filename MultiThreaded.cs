...
namespace Terahard.Assessment.Test {
    class TestContext : IContext {
        ...
        // a slight performance boost, approx 33% faster (3ms down to 2ms in my tests)
        ...
        // Note that phases 1, 2 and 4 are called 250 times each, but phase 3 is only called 249 times.
        ...
        public void Update(float deltaTime, int phase) {
            ...
            // Parallel.For seems to be slightly more performant than Parallel.ForEach
            Parallel.For(0, m_Objects.Count, index => {
                m_Objects[index].Update(phase, lerp);
            });
        }
